import { yearlySalaryData } from "@/models/salary/dtoSalary";
...
interface YearlySalaryChartProps {
  data: yearlySalaryData[];
  showSalary?: boolean;
  onToggleVisibility?: () => void;
}

const YearlySalaryChart: React.FC<YearlySalaryChartProps> = ({
  data,
  showSalary = false,
  onToggleVisibility,
}) => {
  ...
  // Extract month from date string "MM/YYYY"
  ...
  const minSalary = Math.min(...salaryData.filter((s) => s > 0));
  const totalSalary = salaryData.reduce((a, b) => a + b, 0);
  const avgSalary = totalSalary / data.length;

  // Count months with OT and fines
  const monthsWithOT = data.filter((item) => item.otSalary > 0).length;
  const monthsWithFine = data.filter((item) => item.totalFine > 0).length;
  const totalWorkDays = data.reduce((sum, item) => sum + item.totalWorkDay, 0);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View>
            <Text style={styles.title}>Biểu đồ lương theo tháng</Text>
      ...
      <View style={styles.quickStats}>
        <View style={styles.quickStatItem}>
          ...
          <Text style={styles.quickStatLabel}>Tháng</Text>
        </View>
      ...
      {/* Insights */}
      {(monthsWithOT > 0 || monthsWithFine > 0 || totalWorkDays > 0) && (
        <View style={styles.insightsContainer}>
          <Text style={styles.insightsTitle}>Thông tin thêm</Text>
          <View style={styles.insightsRow}>
            {totalWorkDays > 0 && (
              <View
                style={[styles.insightBadge, { backgroundColor: "#DBEAFE" }]}
              >
                <MaterialCommunityIcons
                  name="calendar-check"
                  size={14}
